...
import com.vasyl.kafka.config.AppConstants;
import com.vasyl.kafka.config.AppSerde;
import com.vasyl.kafka.model.Order;
...
public class OrderListenerService {
    ...
    @Bean
    public Function<KStream<String, String>, KStream<String, Order>[]> process() {
        ...
        return input -> {
            KStream<String, OrderEnvelop> input1 = input
                    .peek((k, v) -> log.info(String.format("Received XML Order Key: %s, Value: %s", k, v)))
                    .map((key, value) -> {
            ...
            input1.filter((k, v) -> !k.equalsIgnoreCase(AppConstants.VALID_ORDER))
                    .to(ERROR_TOPIC, Produced.with(AppSerde.String(), AppSerde.orderEnvelop()));

            return input1.filter((k, v) -> k.equalsIgnoreCase(AppConstants.VALID_ORDER))
